...
function dps_log(string $message): void
{
    error_log('[DPS] ' . date('dMy H:i:s') . ' ' . $message);
}
...
function dps_query($conn, string $sql, array $params = [])
{
    ...
    if ($result === false) {
        dps_log('Database query failed: ' . (pg_last_error($conn) ?: 'unknown PostgreSQL error'));
        throw new RuntimeException('Dynamic profile database operation failed.');
    }
    return $result;
}
...
function dps_json($value): array
{
    ...
    if (json_last_error() !== JSON_ERROR_NONE) {
        // dps_log('Invalid JSON state ignored: ' . json_last_error_msg());
        return [];
    }
...
function dps_store($conn, string $key, array $value): void
{
    dps_query(
        $conn,
        'INSERT INTO public.conf_opts(id,value) VALUES($1,$2) ON CONFLICT(id) DO UPDATE SET value=EXCLUDED.value',
...
{
    if ($gamets <= 0) {
        dps_log('Clock update ignored: invalid game time ' . $gamets);
        return;
    }
    ...
    try {
        ...
        dps_log('Unexpected clock update error: ' . $e->getMessage());
        throw $e;
    }
}
...
{
    $limits = [
        'DYNAMIC_PROFILE_INTERVAL_DAYS' => [1, 1 / 24, 365],
        'DYNAMIC_PROFILE_MIN_EVENTS' => [30, 1, 10000],
        'DYNAMIC_PROFILE_COOLDOWN_MINUTES' => [5, 1, 1440]
    ];
    $policy = [];
...
function dps_candidates($conn): array
{
    ...
    foreach ($rows as $row) {
        ...
        if (isset($row['dynamic_profile']) && !in_array($row['dynamic_profile'], [true, 1, '1', 't', 'true'], true))
            $enabled = false;
        $name = trim((string) $row[$product['name']]);
        if ($name === '' || $name === 'The Narrator')
            continue;
...
function dps_audience(array $npc, array &$params): string
{
    ...
    if (dps_product()['prefix'] === 'stobe')
        return stobeEventAudienceSql($npc['name'], $params, [], $npc);
...
function dps_account($conn, array $candidates, array $clock): int
{
    ...
    try {
        ...
        if ((dps_state($conn, 'DYNAMIC_PROFILE_CLOCK')['epoch'] ?? '') !== $clock['epoch']) {
            dps_log('Event accounting aborted: clock epoch changed.');
        ...
        pg_query($conn, 'ROLLBACK');
        dps_log('Unexpected event accounting error: ' . $e->getMessage());
        throw $e;
    }
}
...
function dps_request(array $names): int
{
    ...
    try {
        ...
        dps_log('Unexpected manual request error: ' . $e->getMessage());
        throw $e;
    } finally {
        pg_close($conn);
    }
...
function dps_connector_ready(array $npc): bool
{
    if (dps_product()['prefix'] === 'stobe') {
        ...
        $ready = trim((string) ($config['api_key'] ?? '')) !== '' && trim((string) ($config['model'] ?? '')) !== '';
        if (!$ready)
    ...
    if (function_exists('chimIsGlobalLlmConnectorEnabled') && !chimIsGlobalLlmConnectorEnabled('CORE_CONNECTOR_PROFILES')) {
        dps_log("Connector profiles disabled for {$npc['name']}.");
        return false;
    }
...
{
    error_log('[DPS] ' . date('dMy H:i:s') . ' Scheduler started' . ($manualName ? " for $manualName" : '') . '.');
    $result = ['updated' => 0, 'npcs' => 0, 'events' => 0];
    $conn = $connection ?? ptp_connect();
    if (!$conn) {
        dps_log('Scheduler stopped: database connection unavailable.');
        return $result;
    }
    $locked = false;
    $manualAttempt = null;
    ...
    try {
        ...
        $clock = dps_state($conn, 'DYNAMIC_PROFILE_CLOCK');
        if (!$clock) {
            dps_log('Scheduler stopped: dynamic profile clock is unavailable.');
            return $result;
        }
        if (pg_fetch_result(dps_query($conn, "SELECT EXISTS(SELECT 1 FROM information_schema.columns WHERE table_schema='public' AND table_name='eventlog' AND column_name='dynamic_profile_pending')"), 0, 0) !== 't') {
            dps_log('Scheduler stopped: eventlog.dynamic_profile_pending column is missing.');
            return $result;
        }
        $candidates = dps_candidates($conn);
        $result['events'] = dps_account($conn, $candidates, $clock);
        if ((dps_state($conn, 'DYNAMIC_PROFILE_CLOCK')['epoch'] ?? '') !== $clock['epoch']) {
            dps_log('Scheduler stopped: clock epoch changed during event accounting.');
            return $result;
        }
        $allowed = dps_product()['prefix'] . 'InteractionAllowed';
        if (!$allowed()) {
            dps_log('Scheduler stopped: interaction is not allowed.');
            return $result;
        }
        if (time() - (int) ($clock['seen'] ?? 0) > 300) {
            dps_log('Scheduler stopped: dynamic profile clock is stale.');
            return $result;
        }
        foreach ($candidates as &$npc) {
        ...
        if (sizeof($candidates) === 0) {
            dps_log('Scheduler skipped: no candidates available.');
            return $result;
        }
        foreach ($candidates as $npc) {
            if (!$npc['enabled'] || ($manualName !== null && $npc['name'] !== $manualName)) {
                if ($manualName !== null && $npc['name'] === $manualName)
                    dps_log("Manual request skipped for {$npc['name']}: profile is disabled or has no fields.");
                continue;
            }
            $state = $npc['state'];
...
function dps_generate(array $npc, string $history, string $middleterm = ''): array
{
    if (dps_product()['prefix'] === 'stobe') {
        ...
        if (empty($generated['ok']) || count($generated['updates'] ?? []) !== count($npc['fields'])) {
            dps_log("Stobe profile generator returned an invalid result for {$npc['name']}.");
            return [];
        }
    ...
    foreach ($npc['fields'] as $field) {
        $allowed = dps_product()['prefix'] . 'InteractionAllowed';
        if (!$allowed()) {
            dps_log("Profile generation stopped for {$npc['name']}: interaction is not allowed.");
            return [];
        }
        $value = updateDynamicProfileField($npc['name'], $field, $history, $middleterm);
        if ($value === false || trim((string) $value) === '') {
            dps_log("Profile field generation failed for {$npc['name']}, field '{$field}'.");
            return [];
        }
        if ($field === 'skills' && function_exists('getInGameSkillDataFor'))
...
{
    if (count($updates) !== count($npc['fields']))
        throw new RuntimeException('Incomplete generated profile.');
    $product = dps_product();
    $sets = [];
    $values = [];
    foreach ($updates as $field => $value) {
        if (!in_array($field, $npc['fields'], true) || !is_string($value) || trim($value) === '')
            throw new RuntimeException('Invalid generated profile field.');
        if (!$npc['id']) {
            if ($field === 'backstory')
                $field = 'background';
    ...
    if ($npc['id']) {
        ...
        if ($product['prefix'] === 'stobe') {
            ...
            if (!$snapshot)
                throw new RuntimeException('Cannot prepare NPC recovery history.');
            ...
            dps_query(
                $conn,
                'INSERT INTO public.core_npc_master_history(' . implode(',', $columns) . ') VALUES(' . implode(',', $parameters) . ')',
        ...
        $values[] = $gamets;
        $sets[] = 'gamets_last_updated=$' . count($values);
        $values[] = (int) $npc['id'];
        $saved = dps_query($conn, "UPDATE public.{$product['npc_table']} SET " . implode(',', $sets) . ' WHERE id=$' . count($values), $values);
        if (pg_affected_rows($saved) !== 1)
            throw new RuntimeException('NPC changed during profile update.');
    } else {
