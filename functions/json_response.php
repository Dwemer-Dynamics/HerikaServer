        ...
        if (isset($GLOBALS["FEATURES"]["MISC"]["JSON_DIALOGUE_FORMAT_REORDER"])&&($GLOBALS["FEATURES"]["MISC"]["JSON_DIALOGUE_FORMAT_REORDER"])) {
            if (isset($GLOBALS["LANG_LLM_XTTS"])&&($GLOBALS["LANG_LLM_XTTS"])) {
                $GLOBALS["responseTemplate"] = [
                    ...
                    "item"=>"item identifier (REQUIRED for GiveItemTo: use exact BaseID:ItemName from inventory; for Take_Held_Item: use exact RefID:ItemName from shown in <held_items>, for PickupItem: use exact RefID:ItemName from nearby_items; for CastSpell: use exact spell name from spells; for ReadBook: use exact BaseID:BookTitle from inventory when listed, otherwise use the closest title words from the player's request) OR amount of gold (REQUIRED when action is GiveGoldTo - number as string, e.g. '50') OR destination location name (REQUIRED when action is TeleportNPC) OR item name from the descriptions database (REQUIRED when action is SpawnItem). Leave blank when the chosen action does not need an item, including SpawnGold and SpawnNPC and CreateNewNPC and DirectorCommand.",
            ...
            if (isset($GLOBALS["LANG_LLM_XTTS"])&&($GLOBALS["LANG_LLM_XTTS"])) {
                ...
                $GLOBALS["responseTemplate"] = [
                    ...
                    "item"=>"item identifier (REQUIRED for GiveItemTo: use exact BaseID:ItemName from inventory; for Take_Held_Item: use exact RefID:ItemName from shown in <held_items>, for PickupItem: use exact RefID:ItemName from nearby_items; for CastSpell: use exact spell name from spells; for ReadBook: use exact BaseID:BookTitle from inventory when listed, otherwise use the closest title words from the player's request) OR destination location name (REQUIRED when action is TeleportNPC) OR item name from the descriptions database (REQUIRED when action is SpawnItem). Leave blank when the chosen action does not need an item, including SpawnGold and SpawnNPC and CreateNewNPC.",
